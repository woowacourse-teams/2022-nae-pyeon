...
public interface TeamParticipationRepository extends JpaRepository<TeamParticipation, Long> {
    ...
    @Query("select t "
            ...
            + "join p.team t "
            + "where p.member.id = :memberId")
    Page<Team> findTeamsByMemberId(@Param("memberId") final Long memberId, final Pageable pageRequest);
    ...
    @Query("select p.nickname "
            + "from TeamParticipation p "
            + "where p.team.id = :teamId")
    List<String> findAllNicknamesByTeamId(@Param("teamId") final Long teamId);
